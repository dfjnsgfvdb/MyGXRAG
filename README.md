...
本仓库是“面向桂林旅游的网络舆情热点事件监测分析与预警平台”的能力原型开源版本，重点展示知识图谱可视化、实体关系交互、图谱上下文提取、RAG 问答和后端大模型代理等核心能力。

由于真实桂林旅游舆情数据涉及项目资料、业务数据和隐私合规要求，不适合直接开源，仓库中使用豆瓣与 RSVG 两类样例知识图谱作为脱敏演示数据。其中，豆瓣图谱用于验证多类型实体和多关系图谱的展示、筛选与交互能力；RSVG 图谱用于验证专业领域知识组织、图谱上下文增强和 RAG 问答能力。

在真实桂林旅游舆情场景中，可以将样例图谱替换为景区、事件、媒体来源、游客反馈、情感标签、时间地点等舆情实体及其关系数据，从而支撑热点事件关联分析、舆情知识组织和知识增强问答。
